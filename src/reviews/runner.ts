...
async function writeApiReference(
	...
	const reference = `# @vicinae/api reference\n\nReference package version: ${packageJson.version}\n\nDeclared by changed extensions:\n${declaredVersions.length ? declaredVersions.map((entry) => `- ${entry}`).join("\n") : "- Not found in the loaded files"}\n\n## Public export index\n\n${exportIndex}\n\nUse VICINAE_API/**/*.d.ts as the authoritative API surface. Look up the relevant declaration before making any positive or negative claim about the API, and omit claims the declarations do not establish. Recommend verified APIs without speculating about required package upgrades or dependency-installation steps. Do not execute anything in this package reference.\n`;
