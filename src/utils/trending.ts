...
export async function updateTrendingStatus(): Promise<void> {
	...
	const extensionsWithVelocity = extensions.map((ext) => {
		...
		const effectiveStart =
			ext.createdAt > windowStart ? ext.createdAt : windowStart;
		...
		return {
			...
			downloadsPerDay,
			meetsMinimum:
				ext.downloadCount >= TRENDING_CONFIG.MIN_DOWNLOADS &&
	...
	const trendingCount = Math.max(
		1,
		Math.ceil(candidates.length * TRENDING_CONFIG.TOP_PERCENTILE),
	);
	const trendingIds = new Set(
		candidates.slice(0, trendingCount).map((ext) => ext.id),
	);
